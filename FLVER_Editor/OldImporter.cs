...
public static class OldImporter
{
    ...
    public static bool ImportAssimp(FLVER2 flver, string modelFilePath)
    {
        ...
        Dictionary<string, string> conversionTable = new();
        ...
        if (materialCount > 0)
        {
            for (int i = 0; i < md.Materials.Count; ++i)
            {
                ...
                {
                    newMaterial = JsonConvert.DeserializeObject<FLVER2.Material>(JsonConvert.SerializeObject(flver.Materials[i]));
                }
                else
                {
                ...
                newMaterial.Unk18 = flver.Materials[^1].Unk18 + 1;
        ...
        foreach (Mesh m in md.Meshes)
        {
            ...
            List<List<float>> verticesBoneWeights = new();
            if (m.HasBones)
            {
